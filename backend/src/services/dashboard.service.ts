...
export class DashboardService {
  static async getDashboardMetrics() {
    const [
      ...
      prisma.product.findMany({ select: { id: true, name: true, sku: true, currentStock: true, minimumStock: true, unitPrice: true } }),
      prisma.challan.count({ where: { status: ChallanStatus.DRAFT } }),
      prisma.challan.count({ where: { status: ChallanStatus.CONFIRMED } }),
      prisma.invoice.findMany({ select: { status: true, totalAmount: true, createdAt: true } }),
    ...
    const pendingInvoicesCount = invoices.filter((i) => i.status === InvoiceStatus.GENERATED || i.status === InvoiceStatus.PARTIAL).length;
    ...
    const cancelledChallansCount = await prisma.challan.count({ where: { status: ChallanStatus.CANCELLED } });
    ...
    const inactiveCustomers = await prisma.customer.count({ where: { status: CustomerStatus.INACTIVE } });
    ...
    const confirmedChallanItems = await prisma.challanItem.findMany({
      where: { challan: { status: ChallanStatus.CONFIRMED } },
      select: { productNameSnapshot: true, skuSnapshot: true, quantity: true, totalPrice: true },
    });

    const productSalesMap = new Map<string, { name: string; sku: string; totalQuantity: number; totalRevenue: number }>();
